CREATE OR ALTER PROCEDURE dbo.usp_GenerateJobScripts
(
...
BEGIN
    ...
    SELECT @JD_SCRIPT = STRING_AGG(
'
IF EXISTS (SELECT 1 FROM dbo.JOB_DEFINITION WHERE JOB_DEFINITION_ID = ' + 
CAST(jd.JOB_DEFINITION_ID AS VARCHAR) + ')
BEGIN
    PRINT ''JOB_DEFINITION_ID ' + CAST(jd.JOB_DEFINITION_ID AS VARCHAR) + ' already exists - skipping'';
END
ELSE
BEGIN
...
, CHAR(13)+CHAR(10))
    ...
    SELECT @JS_SCRIPT = STRING_AGG(
'
IF EXISTS (SELECT 1 FROM dbo.JOB_SCHEDULE WHERE JOB_SCHEDULE_ID = ' +
CAST(js.JOB_SCHEDULE_ID AS VARCHAR) + ')
BEGIN
    PRINT ''JOB_SCHEDULE_ID ' + CAST(js.JOB_SCHEDULE_ID AS VARCHAR) + ' already exists - skipping'';
END
ELSE
BEGIN
    ...
    (' +
        ...
        CAST(js.JOB_SCHEDULE_ID AS VARCHAR) + ',' +
        CAST(js.JOB_DEFINITION_ID AS VARCHAR) + ',' +
